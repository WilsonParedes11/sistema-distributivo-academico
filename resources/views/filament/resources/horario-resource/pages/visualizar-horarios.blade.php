                ...
                <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">
                        ...
                        @if($tipoVista === 'carrera')
                            {{ $data['carrera_id'] ? \App\Models\Carrera::find($data['carrera_id'])->nombre : '' }}
                            ({{ $data['semestre'] ?? '' }}{{ $data['paralelo'] ?? '' }})
                        @elseif($tipoVista === 'docente')
                            {{ $data['docente_id'] ? \App\Models\Docente::find($data['docente_id'])->user->nombre_completo : '' }}
                        @endif
                    </h3>
                    <p class="text-sm text-gray-600">
                        Período:
                        {{ $data['periodo_academico_id'] ? \App\Models\PeriodoAcademico::find($data['periodo_academico_id'])->nombre : '' }}
                    </p>
                </div>
                                                ...
                                                <div class="
                                                    ...
                                                    @if($tipoVista === 'carrera')
                                                        <div class="text-gray-600 mb-1">
                                                            {{ Str::limit($horarioEnRango->distributivoAcademico->docente->user->nombre_completo, 15) }}
                                                        </div>
                                                    @else
                                                        <div class="text-gray-600 mb-1">
                ...
                <div class="px-4 py-3 bg-gray-50 border-t border-gray-200">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <span
                                class="ml-1 text-gray-900">{{ $horarios->where('tipo_clase', 'practica')->count() }}</span>
                        </div>
                        <div>
                            <span class="font-medium text-gray-700">Laboratorios:</span>
                            <span
                                class="ml-1 text-gray-900">{{ $horarios->where('tipo_clase', 'laboratorio')->count() }}</span>
                        </div>
                    </div>
                </div>
            </div>
